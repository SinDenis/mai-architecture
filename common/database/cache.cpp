...
namespace database
{
    std::mutex _mtx;
    Cache::Cache() {
        std::string host = Config::get().cache_servers().substr(0, Config::get().cache_servers().find(':'));
        std::string port = Config::get().cache_servers().substr(Config::get().cache_servers().find(':') + 1);
